      ...
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          ...
          <div>
            ...
            <div style={{ fontSize: '14px', color: 'rgba(255,255,255,0.78)', marginTop: '2px' }}>Paket almadan önce pilatesle tanış. Hiçbir taahhüt yok.</div>
          </div>
        </div>
        <a href="https://wa.me/905383135720?text=Merhaba!%20Ücretsiz%20tanışma%20seansı%20almak%20istiyorum." target="_blank" rel="noopener noreferrer"
          style={{ background: '#d4af37', color: '#0d1b3e', fontSize: '14px', fontWeight: 900, padding: '10px 22px', borderRadius: '999px', textDecoration: 'none', letterSpacing: '0.06em', whiteSpace: 'nowrap' }}>
          ÜCRETSİZ DERS AL →
        </a>
      </motion.div>
      ...
      <div style={{ background: G.bg, fontFamily: 'Montserrat,sans-serif', minHeight: '100vh' }}>
        <div style={{ padding: '32px 40px', borderBottom: `1px solid ${G.goldBorder}`, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            <span style={{ fontSize: '14px', fontWeight: 900, letterSpacing: '0.06em', color: type === 'b' ? G.gold : G.whiteMid }}>BİREYSEL</span>
            ...
            <span style={{ fontSize: '14px', fontWeight: 900, letterSpacing: '0.06em', color: type === 'g' ? G.gold : G.whiteMid }}>GRUP</span>
          </div>
        </div>
        ...
        <div className='price-grid' style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', borderBottom: `1px solid ${G.goldBorder}` }}>
          {plans.map((plan, i) => (
            <motion.div key={plan.name}
              ...
              <div style={{ fontSize: '14px', fontWeight: 900, letterSpacing: '0.12em', color: G.gold, marginBottom: '4px' }}>{plan.name}</div>
              <div style={{ fontSize: '14px', color: G.whiteMid, marginBottom: '20px' }}>{plan.sub}</div>
              <div style={{ marginBottom: '6px' }}>
                <span style={{ fontSize: '48px', fontWeight: 900, color: '#fff' }}>₺{plan.price[type]}</span>
                <span style={{ fontSize: '14px', color: G.whiteMid, marginLeft: '4px' }}>/ay</span>
              </div>
              <div style={{ fontSize: '14px', fontWeight: 900, letterSpacing: '0.06em', color: G.whiteMid, marginBottom: '24px' }}>{plan.sessions}</div>
              <ul style={{ listStyle: 'none', padding: 0, flex: 1, marginBottom: '24px' }}>
                {plan.features.map((f, j) => (
                  <li key={j} style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '14px', color: G.whiteMid, marginBottom: '10px' }}>
              ...
              <a href={`https://wa.me/905383135720?text=${encodeURIComponent(`Merhaba! "${plan.name}" paketi hakkında bilgi almak istiyorum.`)}`}
                ...
                style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', padding: '12px', borderRadius: '999px', fontSize: '14px', fontWeight: 900, letterSpacing: '0.06em', cursor: 'pointer', background: plan.popular ? G.gold : 'transparent', color: plan.popular ? G.bg : G.gold, border: `1.5px solid ${G.gold}` }}>
          ...
          {faqs.map((faq, i) => (
            <div key={i} style={{ borderTop: `1px solid ${G.goldBorder}` }}>
              <button onClick={() => setOpenFaq(openFaq === i ? null : i)}
                style={{ width: '100%', textAlign: 'left', padding: '18px 0', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'none', border: 'none', cursor: 'pointer', fontSize: '14px', fontWeight: 900, color: '#fff', fontFamily: 'Montserrat' }}>
                {faq.q}
              ...
              <AnimatePresence>
                {openFaq === i && (
                  <motion.p initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }}
                    style={{ fontSize: '14px', color: G.whiteMid, paddingBottom: '18px', lineHeight: 1.7 }}>
                    {faq.a}
                  </motion.p>
                )}
              </AnimatePresence>
