    ...
    <div style={{ padding: 'clamp(28px,5vw,40px) clamp(16px,4vw,40px)', background: G.dark, borderBottom: `1px solid ${G.goldBorder}`, fontFamily: 'Montserrat,sans-serif' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '24px', flexWrap: 'wrap', gap: '12px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          ...
          <span style={{ fontSize: '9px', fontWeight: 900, letterSpacing: '0.15em', color: G.gold }}>GOOGLE DEĞERLENDİRMELERİ</span>
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          ...
          <div>
            <div style={{ fontSize: '10px', color: 'rgba(255,255,255,0.35)' }}>47 değerlendirme</div>
            <div style={{ display: 'flex', gap: '3px', marginTop: '2px' }}>
              {'GOOGLE'.split('').map((l, i) => (
                <span key={i} style={{ fontSize: '10px', fontWeight: 900, color: ['#4285f4','#ea4335','#fbbc04','#34a853','#4285f4','#ea4335'][i] }}>{l}</span>
              ))}
            </div>
          </div>
        </div>
        ...
        {reviews.map((r, i) => (
          <motion.div key={i}
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '10px' }}>
              <div style={{ width: '34px', height: '34px', borderRadius: '50%', background: G.gold, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '13px', fontWeight: 900, color: G.bg, flexShrink: 0 }}>
                {r.name[0]}
              </div>
              <div>
                <div style={{ fontSize: '11px', fontWeight: 900, color: '#fff' }}>{r.name}</div>
                <div style={{ fontSize: '9px', color: 'rgba(255,255,255,0.3)', marginTop: '1px' }}>{r.time}</div>
              </div>
            </div>
            <div style={{ color: '#f8d000', fontSize: '12px', marginBottom: '8px', letterSpacing: '2px' }}>{'★'.repeat(r.rating)}</div>
            <p style={{ fontSize: '12px', color: 'rgba(255,255,255,0.75)', lineHeight: 1.65, fontStyle: 'italic' }}>"{r.text}"</p>
          </motion.div>
        ))}
      </div>
      ...
      <div style={{ textAlign: 'center' }}>
        <a href="https://g.page/r/gizemhoca/review" target="_blank" rel="noopener noreferrer"
          style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', fontSize: '10px', fontWeight: 900, color: 'rgba(212,175,55,0.6)', textDecoration: 'none', border: '1px solid rgba(212,175,55,0.2)', padding: '7px 16px', borderRadius: '999px', transition: 'all 0.2s', letterSpacing: '0.06em' }}
